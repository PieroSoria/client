...
import 'package:puntos_client/features/search/domain/models/popular_categories_model.dart';
import 'package:puntos_client/features/search/domain/models/search_suggestion_model.dart';
...
class SearchService implements SearchServiceInterface {
  ...
  List<Item>? sortItemSearchList(
    ...
    if (upperValue > 0) {
      searchItemList.removeWhere((product) =>
    ...
    if (isAvailableItems || isDiscountedItems) {
      if (isAvailableItems) {
        searchItemList.removeWhere((product) => !DateConverter.isAvailable(
  ...
  List<Store>? sortStoreSearchList(
    ...
    if (isAvailableStore || isDiscountedStore) {
      if (isAvailableStore) {
        searchStoreList
            .removeWhere((store) => store.open == 0 || !store.active!);
      }
